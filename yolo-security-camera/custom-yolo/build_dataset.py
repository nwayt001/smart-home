...
def move_files(dataset, img_source_dir, label_source_dir, img_dest_dir, label_dest_dir):
    for image, label in dataset:
        shutil.move(os.path.join(img_source_dir, image), os.path.join(img_dest_dir, image))
        shutil.move(os.path.join(label_source_dir, label), os.path.join(label_dest_dir, label))
